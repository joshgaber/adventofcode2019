...
export default class Day12 {
    ...
    extractCoord = (coords: string, point: string) =>
        parseInt(coords.match(new RegExp(point + '=-?\\d+')).shift().match(/-?\d+/).shift());

    mapMoons = (data: string): Moon[] => data.split('\n').map(a =>
        new Moon(
    ...
    hashState = (moons: Moon[], coord: string) => moons.map(a => `p${a.position[coord]}v${a.velocity[coord]}`).reduce((a, b) => a + b, '');

    totalEnergy = (moons: Moon[]) => moons.reduce((a, b) => a + b.totalEnergy, 0);
}
