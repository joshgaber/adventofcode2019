...
export default class Day6 {
    ...
    constructor(data: string) {
        ...
        orbits.forEach(function(orbit: string) {
            const [orbitee, orbiter] = orbit.split(')');
            let $orbiter: Orbit;
            if (! ($orbiter = this.orbitMap.find((em: Orbit) => orbiter === em.name))) {
                $orbiter = { name: orbiter, neighbors: [], children: [], parent: null };
                this.orbitMap.push($orbiter);
            }
            let $orbitee: Orbit
            if (! ($orbitee = this.orbitMap.find((em: Orbit) => orbitee === em.name))) {
    ...
    getChildrenDepth = (o: Orbit): number => o.children.reduce((a, b) => a + this.getChildrenDepth(b), o.children.length);
