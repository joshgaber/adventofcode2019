...
export default class Day3 {
  ...
  constructor(data: string) {
    ...
    const locationHash = this.convertToHashArray(
      coords,
      (i) => `x${i.x}y${i.y}`
    )
    const crossovers: CoordinateHash = {}
    for (const i in locationHash) {
      if (locationHash.hasOwnProperty(i)) {
        if (
          locationHash[i].find((j) => j.id === 'wire1') &&
          locationHash[i].find((j) => j.id === 'wire2')
        ) {
    ...
    for (const v in vectors) {
      if (vectors.hasOwnProperty(v)) {
        ...
        for (let i = 0; i < distance; i++) {
          const lastPoint = Object.assign({}, points[points.length - 1])
          switch (direction) {
            case 'U':
              lastPoint.y++
              break
  ...
  sortWiresBySteps = (a: Wire, b: Wire): number => a.stepTotal - b.stepTotal

  distance = (point: Coordinates): number =>
    Math.abs(point.x) + Math.abs(point.y)

  convertToHashArray(
    coords: Coordinates[],
    indexer: (i: Coordinates) => string
  ): CoordinateHash {
    const hash: CoordinateHash = {}
