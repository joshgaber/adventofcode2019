export default class Day4 {
  ...
  constructor(data: string) {
    ...
    const range = Array.from(Array(1 - first + last).keys())
      .map((i) => i + first)
      .map(this.intToNumberArray)
  ...
  alwaysIncreases(number: number[]): boolean {
    for (let x = 0; x < 5; x++) {
  ...
  hasDuplicate(number: number[]): boolean {
    for (let x = 0; x < 5; x++) {
      if (number[x] === number[x + 1]) {
        return true
      }
  ...
  hasDuplicateButNotTriplicate(number: number[]): boolean {
    for (let x = 0; x < 5; x++) {
      if (number[x] === number[x + 1]) {
