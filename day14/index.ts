...
export default class Day14 {
  ...
  buildFormulae = (data: string) => data.split(/\r?\n/g).reduce((a, l) => ({
    ...a,
    [l.split(' => ')[1].split(' ')[1]]: {
  ...
  buildReactants = (reactants: string) => reactants.split(/, */g).map(l =>
      ({
        name: l.split(' ')[1],
  ...
  clearReserve = () => Object.keys(this.formulae).forEach(f => this.formulae[f].reserve = 0);
    ...
    return product.reactants.reduce((a, r) =>
        a + (
            r.name === 'ORE' ?
                ...
                this.findOreRequired(this.formulae[r.name], reactions * r.quantity)
        )
        , 0);
  }
}
