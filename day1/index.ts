...
export default class Day1 implements Day {
    ...
    part1(): void {
        let transformed: number[] = this.modules.map(this.calculateFuel.bind(this));
        let total = transformed.reduce((a, b) => a + b);
        console.log('Total weight of fuel needed (base):', total);
    }

    part2(): void {
        const transformed: number[] = this.modules.map(this.calculateWeightOfFuel.bind(this));
